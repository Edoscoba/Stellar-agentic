...
fn setup() -> (Env, EscrowClient<'static>, Address, Address, Address, Address) {
    let env = Env::default();
    env.mock_all_auths();
...
fn arbiter_rules_for_requester() {
    ...
    let balance_before = token_client.balance(&requester); // 10000 - 1000 = 9000
...
fn dispute_timeout_fallback_fails_early() {
    ...
    env.ledger().with_mut(|li| li.sequence += DISPUTE_TIMEOUT_LEDGERS / 2);
...
fn dispute_timeout_fallback_refunds() {
    ...
    env.ledger().with_mut(|li| li.sequence += DISPUTE_TIMEOUT_LEDGERS + 1);
